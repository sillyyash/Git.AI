...
class CoderAgent:
    ...
    def execute(
        ...
        merged_changes = self._deduplicate_changes(all_changes)

        failed_steps = len(errors)
        successful_steps = len(coder_steps) - failed_steps

        summary = (
            f"Executed {len(coder_steps)} coder step(s). "
            f"Succeeded: {successful_steps}. "
            f"Failed: {failed_steps}. "
            f"Generated {len(merged_changes)} change object(s) across "
